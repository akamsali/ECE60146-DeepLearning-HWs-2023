...
class SGDplus(ComputationalGraphPrimer):
    ...
    def backprop_and_update_params(self, y_error, vals_for_input_vars, deriv_sigmoid):
        ...
        v = [0] * (len(self.vals_for_learnable_params))  ## Initialize the velocity vector to all 0's
        for i,param in enumerate(self.vals_for_learnable_params):
            ## Calculate the next step in the parameter hyperplane
            v[i] = self.momentum * v[i] +  (y_error * vals_for_input_vars_dict[input_vars[i]] * deriv_sigmoid)
            step = self.learning_rate * v[i]    
    ...
    def train(self, training_data, mu=None):
        ...
        for i in range(self.training_iterations):
            ...
            self.backprop_and_update_params(y_error_avg, data_tuple_avg, deriv_sigmoid_avg)     ## BACKPROP loss
        # plt.figure()     
